...
- **Scheduling / "due today":** routines carry optional schedule targets (`schedule_weekday` for weekly, `schedule_monthday` for monthly; daily = every day; ad_hoc = never). `lib/data/schedule.ts` derives, for the active location and a day, each active non-ad_hoc routine's status — **done** (a completed run that day), **due** (scheduled today, not done), or **overdue** (a scheduled day missed within a short look-back window) — computing "today" in the location's timezone via `Intl` and bucketing completed runs into local days in JS (no DB scheduler, no cron). The dashboard shows a "Due today" list with a quick "Start run" action.
- **Notifications, digests & email (`/app/notifications`):** an in-app inbox plus optional email delivery. A user reads/marks-read only their own notifications (RLS) and manages their own `notify_email` opt-in. The **due digest** for a location is generated by `lib/data/digest.ts` (`generateAndSendDigest`): it computes due/overdue app-side (`lib/data/schedule.ts` — the single source of truth), creates one in-app notification per owner/manager idempotently per day (`dedupe_key`), and emails opted-in recipients via **Resend** (`lib/email/resend.ts`, plain `fetch`, no SDK), recording `emailed_at` so re-runs never double-send. If `RESEND_API_KEY`/`EMAIL_FROM` are unset, it stays in-app-only. A manager/owner can trigger it on demand from the inbox; the automated path is a secret-protected route handler **`POST /api/cron/due-digest`** that loops all active locations, scheduled daily by an external trigger (e.g. Supabase `pg_cron` + `pg_net`) — inert until `CRON_SECRET` is set. The app shell shows an unread-count bell.

  **Sanctioned service-role use:** digest generation writes notifications for *other* users and reads their emails, which RLS forbids, so it runs with the **admin (service-role) client** — the one deliberate system-level use of that key. It is only reached server-side: from the shared-secret cron route, or from an app action *after* the caller is verified as a manager/owner. The key is never exposed to the browser; all user-facing reads/writes remain RLS-governed.

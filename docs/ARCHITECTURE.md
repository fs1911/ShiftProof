...
- **Notifications & digests (`/app/notifications`):** an in-app inbox (no external delivery yet). A user reads/marks-read only their own notifications (RLS); rows are created solely by the `create_digest_notifications` `SECURITY DEFINER` function. A manager/owner can generate today's **due digest** for the active location: the due/overdue figures are computed app-side (`lib/data/schedule.ts`, the single source of truth for scheduling rules) and the function fans a one-per-owner/manager summary out idempotently per day (`dedupe_key`). The app shell shows an unread-count bell. A future scheduled job would call an app endpoint that runs the same computation and function — no cron or provider is wired yet.
